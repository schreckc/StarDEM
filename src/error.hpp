...
#define TR(debug_info) if (__MODE_DEBUG >= 3) {std::cout<<debug_info<<std::endl;}

#define TEST(cond) if (!(cond)) {close(); assert(cond);}

#define IS_DEF(number) assert(!std::isnan(number) && !std::isinf(nombre))

//*********************

#else

#define ERROR(cond, msg, debug_info) if (!(cond)) {std::cerr<<"\n***ERREUR*** : \n"<<msg<<"\n"<<std::endl; std::exit(-1);}

#define WARNING(cond, msg, debug_info) if (!(cond)) {std::cerr<<"\n****WARNING*** : \n"<<msg<<"\n"<<std::endl;}

#define INFO(level, debug_info) if (__MODE_DEBUG >= level) {std::cout<<"INFO: \n"<<debug_info<<std::endl;}

#define TR(debug_info)

#define TEST(cond)

#define IS_DEF(number)
